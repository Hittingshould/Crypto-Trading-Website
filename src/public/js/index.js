...
import { accountFrozenModal, showAlert } from './alerts';
...
const updateTransactionBtn = document.querySelectorAll(
...
const updatePrices = async () => {
	...
	if (prices) {
		...
		document.getElementById(
			'btc-price'
		).innerText = `$${prices.bitcoin.usd.toFixed(2)}`;
		document.getElementById(
			'btc-change'
		).innerText = `${prices.bitcoin.usd_24h_change.toFixed(2)}%`;

		// Update ETH price
		document.getElementById(
			'eth-price'
		).innerText = `$${prices.ethereum.usd.toFixed(2)}`;
		document.getElementById(
			'eth-change'
		).innerText = `${prices.ethereum.usd_24h_change.toFixed(2)}%`;

		// Update LTC price
		document.getElementById(
			'ltc-price'
		).innerText = `$${prices.litecoin.usd.toFixed(2)}`;
		document.getElementById(
			'ltc-change'
		).innerText = `${prices.litecoin.usd_24h_change.toFixed(2)}%`;
	}
};
	...
	forgotPasswordForm.addEventListener('submit', async (e) => {
		...
		try {
			...
			if (res.status === 200) {
				showAlert(
					'success',
					'If the address is registered, we’ve emailed reset instructions.'
				);
				window.setTimeout(() => {
